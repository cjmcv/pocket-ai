...
void Im2Col(DType* dst, const DType* src, const int channels, 
    ...
    for (int ky = 0, h_offset = 0; ky < kernel_h; ky++, h_offset += dilation_h) {
        ...
        for (int kx = 0, w_offset = 0; kx < kernel_w; kx++, w_offset += dilation_w) {
            ...
            int oh_end =
                std::min(((input_h + pad_bottom - h_offset + stride_h - 1) / stride_h), output_h);
            oh_end = std::max(oh_begin, oh_end);
            int ow_begin = std::max(((pad_left - w_offset + stride_w - 1) / stride_w), 0);
            int ow_end =
                std::min(((input_w + pad_right - w_offset + stride_w - 1) / stride_w), output_w);
            ...
            for (int oh = oh_begin; oh < oh_end; ++oh, ih += stride_h) {
                ...
                for (int ow = ow_begin; ow < ow_end; ++ow, iw += stride_w) {
                    ...
                    DType* dst_ptr = dst + (oh * output_w + ow) * kernel_h * kernel_w * channels
                                    + dst_ky_offset + dst_kx_offset;
                    /* NHWC --> NHWRRC */
...
void Col2Im(DType* dst, const DType* src, const int channels,
    ...
    const int col_height =
        (output_h + pad_h0 + pad_h1 - (dilation_h * (kernel_h - 1) + 1)) / stride_h + 1;
    const int col_width =
        (output_w + pad_w0 + pad_w1 - (dilation_w * (kernel_w - 1) + 1)) / stride_w + 1;
    // GEMM: NHWC * C'RRC(CC'RR) = NHWC'RR
    // Col2Im: N HWC'RR --> N H'W'C'
    // GEMM: NHWC * C'RRC(CRRC') = NHWRRC'
    // Col2Im: N HWRRC' --> N H'W'C'(vectorize optimize)
        ...
        for (int kw = 0; kw < kernel_w; kw++) {
            ...
            for (int col_h = 0; col_h < col_height; col_h++) {
                ...
                for (int col_w = 0; col_w < col_width; col_w++) {
                    if (im_h >= 0 && im_h < output_h &&
                        im_w >= 0 && im_w < output_w) {
...
void GemmTransBAddBias(const int M, const int N, const int K, 
...
void GemmTransBAddBiasQuant(const int M, const int N, const int K, 
                            const int8_t *A, const int8_t *B, int32_t *C, 
                            const int32_t *bias, const int8_t input_zero_point) {
    for (int i=0; i<M; i++) {
        for (int j=0; j<N; j++) {
            C[i*N+j] = bias[j];
            for (int k=0; k<K; k++) {
                C[i*N+j] += ((int32_t)A[i*K+k] - input_zero_point) * B[j*K+k];
            }
        }
    }
}
